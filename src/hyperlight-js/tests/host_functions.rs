...
fn register_js_nested_binary_return_in_object() {
    // Host returns an object with a nested buffer → guest sees Uint8Array
    let handler = Script::from_content(
        r#"
        import * as host from "host";
        function handler(event) {
    ...
    proto.host_module("host").register_js(
        "get_payload",
        |_args: serde_json::Value, _blobs: Vec<Vec<u8>>| {
            // Return JSON + sidecar: {"name": "test", "data": {"__bin__": 0}}
            let sidecar = hyperlight_js_common::encode_binaries(&[&[10u8, 20, 30][..]])
                .map_err(|e| hyperlight_js::HyperlightError::Error(format!("{e}")))?;
            ...
            let json_str = serde_json::to_string(&json)
                .map_err(|e| hyperlight_js::HyperlightError::Error(format!("{e}")))?;
            Ok(hyperlight_js::FnReturn::JsonWithBinaries(json_str, sidecar))
        },
    );
    ...
    assert_eq!(res, r#"{"name":"test","data_len":3,"first":10,"last":30}"#);
}

#[test]
fn register_js_nested_binary_return_in_array() {
    // Host returns an array containing buffers → guest sees Uint8Arrays
    let handler = Script::from_content(
        r#"
        import * as host from "host";
        function handler(event) {
    ...
    proto.host_module("host").register_js(
        "get_items",
        |_args: serde_json::Value, _blobs: Vec<Vec<u8>>| {
            // Return an array of two binary blobs
            let sidecar = hyperlight_js_common::encode_binaries(&[&[1u8, 2, 3][..], &[4u8, 5][..]])
                .map_err(|e| hyperlight_js::HyperlightError::Error(format!("{e}")))?;
            let json = serde_json::json!([{"__bin__": 0}, {"__bin__": 1}]);
            let json_str = serde_json::to_string(&json)
                .map_err(|e| hyperlight_js::HyperlightError::Error(format!("{e}")))?;
            Ok(hyperlight_js::FnReturn::JsonWithBinaries(json_str, sidecar))
        },
    ...
    assert_eq!(
        res,
        r#"{"count":2,"first_len":3,"second_len":2,"first_byte":1}"#
    );
}

#[test]
fn register_js_deeply_nested_binary_return() {
    // Host returns a deeply nested structure with binary data
    let handler = Script::from_content(
        r#"
        import * as host from "host";
        function handler(event) {
    ...
    proto.host_module("host").register_js(
        "get_nested",
        |_args: serde_json::Value, _blobs: Vec<Vec<u8>>| {
            let sidecar = hyperlight_js_common::encode_binaries(&[&[0xABu8, 0xCD][..]])
                .map_err(|e| hyperlight_js::HyperlightError::Error(format!("{e}")))?;
            let json = serde_json::json!({
                "outer": {
                    "inner": {
